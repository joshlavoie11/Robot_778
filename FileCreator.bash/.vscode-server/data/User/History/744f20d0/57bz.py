...
class TwistToMotorPublisher(Node):
    ...
    def cmd_vel_callback(self, msg):
        ...
        linearSpeed = [self.convertSpeedLeft(self, v_lin_x, w_ang_z), self.convertSpeedRight(self, v_lin_x, w_ang_z)]

        self.speedCommand = self.v_LinToTicks(linearSpeed)
        self.publisher_.publish(self.speedCommand)

    def encoder_a_callback(self, msg):
        currentTickCount = msg
    ...
    def encoder_b_callback(self, msg):
        ...
        self.get_logger().info(f"Roue gauche - Consigne: {self.speedCommand[1]:.3f} m/s, Mesure: {msSpeed:.3f} m/s, Erreur: {error:.3f} m/s")
